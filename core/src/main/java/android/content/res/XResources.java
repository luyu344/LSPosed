...
import static de.robv.android.xp0sed.Xp0sedHelpers.decrementMethodDepth;
import static de.robv.android.xp0sed.Xp0sedHelpers.findAndHookMethod;
import static de.robv.android.xp0sed.Xp0sedHelpers.getBooleanField;
import static de.robv.android.xp0sed.Xp0sedHelpers.getLongField;
import static de.robv.android.xp0sed.Xp0sedHelpers.getObjectField;
import static de.robv.android.xp0sed.Xp0sedHelpers.incrementMethodDepth;
...
import de.robv.android.xp0sed.IXp0sedHookZygoteInit;
import de.robv.android.xp0sed.XC_MethodHook;
import de.robv.android.xp0sed.XC_MethodHook.MethodHookParam;
import de.robv.android.xp0sed.Xp0sedBridge;
import de.robv.android.xp0sed.Xp0sedBridge.CopyOnWriteSortedSet;
import de.robv.android.xp0sed.Xp0sedInit;
import de.robv.android.xp0sed.callbacks.XC_LayoutInflated;
import de.robv.android.xp0sed.callbacks.XC_LayoutInflated.LayoutInflatedParam;
...
//			Log.w(XposedBridge.TAG, "Package name for " + resDir + " had to be retrieved via parser");
	 ...
	 * {@link IXp0sedHookZygoteInit#initZygote initZygote()} for all apps. As there is no
			...
			if (incrementMethodDepth("getDrawable") == 1) {
				...
				if (replacement instanceof DrawableLoader) {
					try {
						...
						if (result != null)
							return result;
					} catch (Throwable t) { Xp0sedBridge.log(t); }
				} else if (replacement instanceof Integer) {
			...
			if (callbacks != null) {
				...
				if (value.type == TypedValue.TYPE_STRING) {
					...
					if (components.length == 3)
						...
						Xp0sedBridge.log("Unexpected resource path \"" + value.string.toString()
								+ "\" for resource id 0x" + Integer.toHexString(id));
				} else {
					Xp0sedBridge.log(new NotFoundException("Could not find file name for resource id 0x") + Integer.toHexString(id));
				}
	...
	public void getValue(int id, TypedValue outValue, boolean resolveRefs) throws NotFoundException {
		...
		if (replacement instanceof XResForwarder) {
			...
			if (replacement != null) {
				Xp0sedBridge.log("Replacement of resource ID #0x" + Integer.toHexString(id) + " escaped because of deprecated replacement. Please use XResForwarder instead.");
			}
	...
	private static int translateResId(int id, XResources origRes, Resources repRes) {
		try {
			...
			if (!repResDefined && origResId == 0 && !entryType.equals("id")) {
				Xp0sedBridge.log(entryType + "/" + entryName + " is neither defined in module nor in original resources");
				return 0;
			}
	...
	private static int translateAttrId(String attrName, XResources origRes) {
		...
		try {
			...
			Xp0sedBridge.log("Attribute " + attrName + " not found in original resources");
		}
		return origAttrId;
	}
