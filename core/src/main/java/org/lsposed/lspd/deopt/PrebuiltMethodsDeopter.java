        ...
        for (Object[] caller : callers) {
            try {
                ...
                if ("<init>".equals(caller[1])) {
                    method = Xp0sedHelpers.findConstructorExactIfExists((String) caller[0], cl, params);
                } else {
                    method = Xp0sedHelpers.findMethodExactIfExists((String) caller[0], cl, (String) caller[1], params);
                }
                if (method != null) {
